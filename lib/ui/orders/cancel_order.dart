...
class CancelOrderPage extends StatefulWidget {
  ...
  CancelOrderPage({Key key, @required this.orderId, @required this.orderStatus})
      : super(key: key);
...
class _CancelOrderPageState extends State<CancelOrderPage> with Constants {
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
        child: Scaffold(
            ...
            appBar: CommonAppBar(
              appBar: AppBar(),
              title: "${AppTranslations.of(context).text("Key_CancelOrder")}",
            ),
  ...
  apiReasonList(
    ...
    Response response = await RestClient.getData(
        context, ApiEndPoints.apiTicketReasonList + "CANCEL", accessToken);
  ...
  apiCancelOrder(
    ...
    if (widget.orderStatus == pending) {
      response = await RestClient.postData(
          context, ApiEndPoints.apiCancelOrder, data, accessToken);
    } else if (widget.orderStatus == returnRequested) {
      response = await RestClient.postData(
          context, ApiEndPoints.apiReturnCancelOrder, data, accessToken);
    } else if (widget.orderStatus == replaceRequested) {
      response = await RestClient.postData(
          context, ApiEndPoints.apiReplaceCancelOrder, data, accessToken);
    }
    ...
    if (response.statusCode == 200) {
      ...
      if (commonResponce.status == ApiEndPoints.apiStatus_200) {
        ...
        providerTrackWatch.updateHomeOrderButton(false);
